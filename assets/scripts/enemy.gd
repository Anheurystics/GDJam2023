...
func _ready():
	...
	load_sprite_frames("walk", sprite_name, walk_frames);
	
	var hurt_frames = [];
	for i in range(num_hurt_frames):
		hurt_frames.push_back(char(hurt_start_frame.to_ascii_buffer()[0] + i));
	load_sprite_frames("hurt", sprite_name, hurt_frames);
...
func _on_nav_agent_link_reached(_details):
...
func load_sprite_frames(anim_prefix: String, prefix: String, frames: Array):
	if frames.is_empty():
		return;
...
func deal_damage():
	nav_paused = true;
	if sprite.sprite_frames.has_animation("hurt_" + curr_anim_suffix):
		sprite.play("hurt_" + curr_anim_suffix);
	if shadeSprite.sprite_frames.has_animation("hurt_" + curr_anim_suffix):
		shadeSprite.play("hurt_" + curr_anim_suffix);
...
func _process(_delta):
	var angle = get_face_angle_to_position(get_viewport().get_camera_3d().global_position);
	var pi_ratio = abs(angle) / PI;
	var flip = angle < 0;
	...
	sprite.play("walk_" + curr_anim_suffix);
	shadeSprite.play("walk_" + curr_anim_suffix);
	
	if velocity.length_squared() > 0.1:
		sprite.speed_scale = 1;
		shadeSprite.speed_scale = 1;
	else:
		sprite.speed_scale = 0;
		sprite.set_frame_and_progress(1, 0);
		shadeSprite.speed_scale = 0;
...
func _physics_process(delta):
	...
	if not nav_agent.is_navigation_finished():
		...
		if global_position.distance_to(target) > 0.1 and not nav_paused:
